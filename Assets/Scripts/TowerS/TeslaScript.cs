...
public class TeslaScript : AbstractTower
{
    public float fireDelay;
    public ElectroBallScript bulletPrefab;
    ...
    void Fire()
    {
        ...
        if (target is null)
            return;
        SoundManager.Instance.PlayElectro();
        timeRemainingBeforeNextFire = fireDelay;
        int bullets = 1;
        if (crabsInRange.Count > 3)
            bullets += specificUpgrades[1];
        if (crabsInRange.Count > 5)
            bullets += 2*specificUpgrades[2];
        if (crabsInRange.Count > 10)
            bullets += 5*specificUpgrades[3];
        for (int i = 0; i < bullets; i++)
        {
            target = ChooseTarget();
            ElectroBallScript bullet = Instantiate(bulletPrefab, transform.position, Quaternion.identity);
            bullet.target = target.gameObject;
            bullet.paralyseTime = 1 + 0.2f * specificUpgrades[0];
        }
